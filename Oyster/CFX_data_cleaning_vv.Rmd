...
```{r}
raw <- read.csv("./6_15_22/vv_data_6_15_22_sample_1.csv") %>%
  select(-target) %>%
  mutate(target = "Vv")
```
...
unkn_hex %>% ggplot() +
  ...
  xlab("Container Type") +
  ylab("Cq Values (N = 3)") +
  ggtitle("Unknown Samples: Cq Values vvh")+
...
controls_clean <- controls_only %>% mutate(dilution = case_when(
  ...
  mutate(replicate = rep(1:3, 26))%>%   #divide error messave number by three then add at the end of rep()
  select(-well) %>%
  rename(well = long_well)
...
control_hex %>% ggplot() +
  ...
  xlab("Container Type") +
  ylab("Cq Values (N = 3)") +
  ggtitle("Controls: Cq Values vvh")+
  labs(fill = "MPN Dilution")+
  scale_y_continuous(breaks = seq(15, 40, 10), limits = c(15, 40))+
...
write.csv(controls_clean, "./11_2_22/clean_controls_vv_11_2_22_cleaned.csv")
write.csv(unkn_clean, "./11_2_22/clean_unknown_vv_11_2_22_cleaned.csv")
...
mpn_frame <- unkn_hex %>% mutate(pos = case_when(
  .$cq > 36 | .$cq == "NaN" ~ "Negative",   #set threshold
  TRUE ~ "Positive"))
...
```{r}
write_csv(master_mpn, "./11_2_22/master_mpn_sample10_vv.csv")
write.csv(mpn_pretty, "./11_2_22/raw_mpn_sample10.csv")
```
