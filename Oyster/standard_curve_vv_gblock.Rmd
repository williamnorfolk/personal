---
title: "standard_curve_gblock_vv"
...
```{r}
raw <- read.csv("./std_curves/vv_gblock_std_curve_good.csv") %>%
  select(-c(target)) %>%
  mutate(target = "Vv")
```
...
clean <- raw %>% mutate(taq = case_when(
  .$sample == "VvSTD plat" ~ "platinum",
  .$sample == "VvSTD Norm" ~ "regular",
  TRUE ~ "water"))
...
hex_plot %>% ggplot() +
  ...
  ylab("Cq Value") +
  ggtitle("G-Block Standard Curve Vv (HEX)")+
  labs(color = "Concentration/\nContent")
```
```{r}
no_controls <- hex_plot %>% subset(content == "Std")
write.csv(no_controls, "./vv_taq_compare.csv")
```
...
cy5_plot %>% ggplot() +
  ...
  xlab("Concentration/Content")+
  ylab("Cq Value") +
  ggtitle("G-Block Standard Curve Vv (Cy5)")+
  labs(color = "Concentration/\nContent")+
