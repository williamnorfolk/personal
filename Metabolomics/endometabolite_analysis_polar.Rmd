...
p100 <- try1 %>% ggplot() +
  ...
  ggtitle("Vibrio alginolyticus Polar (NSFe+/NSFe-)")+
  scale_shape_manual(values = shape_key)

p100

#POLAR S Iron Volcano
p200 <- try2 %>% ggplot() +
  geom_point(aes(x = log2_fold_change, y = neg_log10_pval, color = color_code)) +
  geom_vline(xintercept = 1, linetype = "longdash")+
  geom_vline(xintercept = -1, linetype = "longdash")+
  ...
  labs(color = "Regulation")+
  scale_color_manual(values = colors)+
  ggtitle("Vibrio alginolyticus Polar (SFe+/SFe-)")+
  scale_shape_manual(values = shape_key)
...
p300 <- try3 %>% ggplot() +
  ...
  labs(color = "Regulation")+
  scale_color_manual(values = colors)+
  ggtitle("Vibrio alginolyticus Polar (NSFe+/SFe+)")+
  scale_shape_manual(values = shape_key)
...
```{r}
library(ggthemes)
#name_master_key$retention_time <- as.factor(as.numeric(name_master_key$retention_time))
#top_20_iron_ns_clean <- top_20_iron_ns_clean %>% left_join(name_master_key)
...
```{r}
iron_compare_ns <- try1 %>% select(c(retention_time, p_val, NS_Y, NS_N, up_or_down, color_code)) %>%
  mutate(iron_diff = NS_Y - NS_N)

#zztop<- iron_compare_ns %>% slice_max(order_by = p_val, n=20) %>% 
 # mutate(up_or_down = "Up Regulated")
zzpval<- iron_compare_ns %>% slice_min(order_by = p_val, n=40)


ss <- zzpval %>% select(retention_time, color_code, p_val, iron_diff, up_or_down) %>% left_join(clean_polar) %>% 
  subset(full_name == "NS_Y" | full_name == "NS_N") %>%
  mutate(retention_time = round(retention_time, 4)) %>%
  mutate(iron = recode(iron,
                                 "Y" = "Iron Replete",
...
ss$retention_time <- as.factor(as.numeric(ss$retention_time))


```
**********************************
P-value based code for plots above
**********************************
```{r}
ss %>% ggplot() +
  geom_boxplot(aes(x = reorder(retention_time, iron_diff), y = values, fill = iron)) +
  facet_wrap(~up_or_down, scales = "free")+
  theme_hc() +
  xlab("Retention Time")+
  ylab("Relative Concentration")+
  labs(fill = "Iron Condition")+
  ggtitle("Polar Non-Starved Iron Comparison (NSFe+ vs. NSFe-)")+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, size = 8),
        plot.title = element_text(size = 12),
        axis.title = element_text(size = 11))
...
```
```{r}
write.csv(all_all, "./polar_key_retentions.csv")
```
...
```{r}
write.csv(find_correct_names, "./polar_ids/identify_peaks.csv")
write.csv(remove_overlaps, "./polar_ids/only_wins.csv")
write.csv(all_names, "./polar_ids/important_metabolites.csv")

```


```{r}
write.csv(name_master_key, "./polar_ids/peak_id_list_polar_endometabolites.csv")
```
