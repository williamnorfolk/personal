---
title: "power_lines_plot_part_delta"
...
raw <- readRDS("./raw_data/delta_clean_BWA_amp_09.29.2021.rds")
...
p1 <- clean %>% ggplot() +
  ...
  xlab("Coverage") +
  ylab("Total Errors") +
  ggtitle("B: BWA Assembly + Delta + Amplicons")
...
```{r}
#raw <- read_excel("./raw_data/Delta_Illumina_amp_IRMA_clean_09.15.2021.xlsx")
raw <- readRDS("./raw_data/delta_clean_IRMA_amp_09.28.2021.rds")
#tag total rows in the dataset
total_rows <- nrow(raw)
```
...
p2 <- clean %>% ggplot() +
  ...
  xlab("Coverage") +
  ylab("Total Errors") +
  ggtitle("C: IRMA Assembly + Delta + Amplicons")
...
```{r}
#raw <- read_excel("./raw_data/Delta_Illumina_amp_IRMA_clean_09.15.2021.xlsx")
raw <- readRDS("./raw_data/delta_clean_SPAdes_amp_09.29.2021.rds")
#tag total rows in the dataset
total_rows <- nrow(raw)
```
...
p3 <- clean %>% ggplot() +
  ...
  xlab("Coverage") +
  ylab("Total Errors") +
  ggtitle("A: SPAdes Assembly + Delta + Amplicons")
...
#library(ggpubr)
ggsave(make_this, file = "./figure_2.png")
